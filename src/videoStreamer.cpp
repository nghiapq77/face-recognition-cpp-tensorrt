...
VideoStreamer::VideoStreamer(int nmbrDevice, int videoWidth, int videoHeight, int frameRate, bool isCSICam) {
    if (isCSICam) {
        ...
        std::string pipeline = gstreamer_pipeline(videoWidth, videoHeight, videoWidth, videoHeight, frameRate);
...
void VideoStreamer::getFrame(cv::Mat &frame) { *m_capture >> frame; }
...
std::string VideoStreamer::gstreamer_pipeline(int capture_width, int capture_height, int display_width,
